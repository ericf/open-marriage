...
function ExpressHandlebars(config) {
    ...
    this.layoutDir     = config.layoutDir || this.layoutDir;
...
extend(ExpressHandlebars.prototype, {
    ...
    getPartials: function (options, callback) {
        ...
        function getPartial(filePath, callback) {
            ...
            self.getTemplate(filePath, options, callback);
        }

        async.waterfall([
            function (callback) {
                self._readDir(partialsDir, options, callback);
            },

            function (dir, callback) {
                async.map(dir, getPartial, function (err, partials) {
                    callback(err, dir, partials);
                });
            },

            function (dir, partials, callback) {
                var extRegex    = new RegExp(self.extname + '$'),
                    partialsMap = {};

                dir.forEach(function (filePath, i) {
                    var name = filePath.replace(extRegex, '').replace('/', '.');
    ...
    getTemplate: function (filePath, options, callback) {
        ...
        this._readFile(filePath, options, function (err, file) {
    ...
    renderView: function (viewPath, options, callback) {
        ...
        if (layoutPath[0] !== '/') {
            layoutPath = path.join(this.layoutDir, layoutPath);
        }
    ...
    _readDir: function (dirPath, options, callback) {
    ...
    _readFile: function (filePath, options, callback) {
...
function exphbs(config) {
    return new ExpressHandlebars(config).engine;
}
